...
(function () {
    ...
    function ya(a, c, b) {
        var d, e;
        if (Ib(c)) if (M(b)) a.setAttribute(c, b); else {
            if (a && a.getAttribute) e = a.getAttribute(c)
        } else if (M(c) && Jb(c)) for (d in c) a.setAttribute(d,
            c[d]);
        return e
    }
    ...
    function Ua(a, c) {
        if (Zc) if (c && c.opacity !== Oa) c.filter = "alpha(opacity=" + c.opacity * 100 + ")";
        oa(a.style, c)
    }
    ...
    function fb(a, c, b, d, e) {
        ...
        c && oa(a, c);
        e && Ua(a, {padding: 0, border: mb, margin: 0});
        b && Ua(a, b);
        d && d.appendChild(a);
        return a
    }
    ...
    function Hd(a, c) {
        function b(m, h) {
                    ...
                    if (r) {
                        ...
                        if (o.isCartesian) {
                            t(o.data, function (da) {
                                var B = da.x, sa = da.y, R = sa < 0, X = R ? F : W;
                                R = R ? ea : A;
                                if (Qa === null) Qa = v = da[G];
                                if (ha) if (B > v) v = B; else {
                                    if (B < Qa) Qa = B
                                } else if (M(sa)) {
                                    if (V) X[B] = M(X[B]) ? X[B] + sa : sa;
                                    da = X ? X[B] : sa;
                                    if (!Gc) if (da > v) v = da; else if (da < Qa) Qa = da;
                                    if (V) {
                                        aa[R] || (aa[R] = {});
                                        aa[R][B] = {total: da, cum: da}
                                    }
                                }
                            });
                            if (/(area|column|bar)/.test(o.type) && !ha) if (Qa >= 0) {
                                Qa = 0;
                                Id = true
                            } else if (v < 0) {
                                v = 0;
            ...
            function fa(l, p) {
                ...
                if (!p) {
                    p = [1, 2, 2.5, 5, 10];
                    if (h.allowDecimals === false) if (Eb == 1) p = [1, 2, 5, 10]; else if (Eb <= 0.1) p = [1 / Eb]
                }
            ...
            function ia() {
                var l, p, r, o, V = h.tickInterval, W = h.tickPixelInterval;
                ...
                if (J) {
                    la = [];
                    V = Pa.global.useUTC;
                    var F = 1E3 / nb, A = 6E4 / nb, ea = 36E5 / nb;
                    W = 864E5 /
                        nb;
                    l = 6048E5 / nb;
                    o = 2592E6 / nb;
                    var da = 31556952E3 / nb,
                        B = [["second", F, [1, 2, 5, 10, 15, 30]], ["minute", A, [1, 2, 5, 10, 15, 30]], ["hour", ea, [1, 2, 3, 4, 6, 8, 12]], ["day", W, [1, 2]], ["week", l, [1, 2]], ["month", o, [1, 2, 3, 4, 6]], ["year", da, null]],
                        sa = B[6], R = sa[1], X = sa[2];
                    for (r = 0; r < B.length; r++) {
                        sa = B[r];
                        R = sa[1];
                        X = sa[2];
                        if (B[r + 1]) if (Va <= (R * X[X.length - 1] + B[r + 1][1]) / 2) break
                    }
                    ...
                    for (ea = X[nc](); F < P && r < Ba;) {
                        la.push(F);
                        if (R == da) F = Ac(p + r * B, 0) / nb; else if (R == o) F = Ac(p, A + r * B) / nb; else if (!V && (R == W || R == l)) F = Ac(p, A, ea + r * B * (R == W ? 1 : 7)); else F += R * B;
                        r++
                    }
                    la.push(F);
                ...
                if (!Nb) {
                    ...
                    p = la[0];
                    r = la[la.length - 1];
                    if (h.startOnTick) H = p; else H > p && la.shift();
                    if (h.endOnTick) P = r; else P < r && la.pop();
                    Fb || (Fb = {x: 0, y: 0});
            ...
            function Ea() {
                ...
                ba = D;
                D = C / (P - H || 1);
                if (!ha) for (l in aa) for (p in aa[l]) aa[l][p].cum = aa[l][p].total;
            ...
            function Ya() {
                var l = h.title, p = h.alternateGridColor,
                    r = h.lineWidth, o, V, W = m.hasRendered, F = W && M(hb) && !isNaN(hb);
                o = S.length && M(H) && M(P);
                C = L ? Ba : qa;
                D = C / (P - H || 1);
                ua = L ? U : ob;
                if (o || Nb) {
                    if (Hc && !gb) for (o = H + (la[0] - H) % Hc; o <= P; o += Hc) {
                    ...
                    p && t(la, function (A, ea) {
                        if (ea % 2 === 0 && A < P) {
                            dc[A] || (dc[A] = new w);
                            dc[A].options = {
                                from: A, to: la[ea + 1] !== Oa ? la[ea + 1] : P,
                                color: p
                            };
                            dc[A].render();
                            dc[A].isActive = true
                ...
                t([pb, Wb, dc], function (A) {
                    for (var ea in A) if (A[ea].isActive) A[ea].isActive = false; else {
                        A[ea].destroy();
                        delete A[ea]
                    }
                });
                if (r) {
                    ...
                    if (Fa) Fa.animate({d: o}); else Fa = Y.path(o).attr({
                        stroke: h.lineColor,
                        "stroke-width": r,
                        zIndex: 7
                    }).add()
                ...
                if (s.axisTitle) {
                    ...
                    r = pa(l.style.fontSize ||
                        12);
                    o = {low: o + (L ? 0 : C), middle: o + C / 2, high: o + (L ? C : 0)}[l.align];
            ...
            var ha = h.isX, Ka = h.opposite, L = Ga ? !ha : ha, E = L ? Ka ? 0 : 2 : Ka ? 1 : 3, aa = {};
            h = va(ha ? Jc : fd, [Wd, Xd, Ld, Yd][E], h);
            var s = this, J = h.type == "datetime", Q = h.offset || 0, G = ha ? "x" : "y", C, D, ba, ua = L ? U : ob,
                ma, ra, cb, Ab, Fa, Qa, v, S, wa, Sa, P = null, H = null,
                hb, cc, Vb = h.minPadding, Kd = h.maxPadding, Nb = M(h.linkedTo), Id, Jd, Gc, Md = h.events, gd,
                Ob = [], Va, Hc, Eb, la, pb = {}, Wb = {}, dc = {}, ec, fc, ed, Ic, gb = h.categories,
                Zd = h.labels.formatter || function () {
                    var l = this.value;
                    return Ic ? Kc(Ic, l) : Va % 1E6 === 0 ? l / 1E6 + "M" : Va % 1E3 === 0 ? l / 1E3 + "k" : !gb && l >= 1E3 ? Gd(l, 0) : l
                }, Lc = L && h.labels.staggerLines, Xb = h.reversed,
                Yb = gb && h.tickmarkPlacement == "between" ? 0.5 : 0;
            x.prototype = {
                addLabel: function () {
                    var l = this.pos, p = h.labels,
                        r = !(l == H && !y(h.showFirstLabel, 1) || l == P && !y(h.showLastLabel, 0)), o = this.label;
                    l =
                        Zd.call({
                            isFirst: l == la[0],
                    ...
                    if (o === Oa) this.label = M(l) && r && p.enabled ? Y.text(l, 0, 0).attr({
                        align: p.align,
                        rotation: p.rotation
                    }).css(p.style).add(cb) : null; else o && o.attr({text: l})
                }, getLabelSize: function () {
                    var l = this.label;
                    return l ? (this.labelBBox = l.getBBox())[L ? "height" : "width"] : 0
                }, render: function (l, p) {
                    var r = !this.minor, o = this.label, V = this.pos, W = h.labels, F = this.gridLine,
                        A = r ? h.gridLineWidth : h.minorGridLineWidth, ea = r ? h.gridLineColor :
                        h.minorGridLineColor, da = r ? h.gridLineDashStyle : h.minorGridLineDashStyle, B = this.mark,
                        sa = r ? h.tickLength : h.minorTickLength, R = r ? h.tickWidth : h.minorTickWidth || 0,
                        X = r ? h.tickColor : h.minorTickColor, oc = r ? h.tickPosition : h.minorTickPosition;
                    r = W.step;
                    var qb = p && Mc || La, Pb;
                    ...
                    if (R) {
                        ...
                        if (B) B.animate({d: A}); else this.mark = Y.path(A).attr({
                            stroke: X,
                            "stroke-width": R
                        }).add(cb)
                    }
                    if (o) {
                        ...
                        o[this.isNew ? "attr" : "animate"]({x: Pb, y: qb})
                    }
                    this.isNew = false
                }, destroy: function () {
            ...
            w.prototype = {
                render: function () {
                    var l =
                            this, p = l.options, r = p.label, o = l.label, V = p.width, W = p.to, F, A = p.from,
                        ea = p.dashStyle, da = l.svgElem, B = [], sa, R, X = p.color;
                    R = p.zIndex;
                    var oc = p.events;
                    if (V) {
                        B = ra(p.value, V);
                        p = {stroke: X, "stroke-width": V};
                        if (ea) p.dashstyle = ea
                    } else if (M(A) && M(W)) {
                        A = Ca(A, H);
                        W = ab(W, P);
                        F = ra(W);
                        if ((B = ra(A)) && F) B.push(F[4], F[5], F[1], F[2]); else B = null;
                        p = {fill: X}
                    } else return;
                    if (M(R)) p.zIndex = R;
                    if (da) if (B) da.animate({d: B}, null, da.onGetPath); else {
                        da.hide();
                    ...
                    if (r && M(r.text) && B && B.length && Ba > 0 && qa > 0) {
                        ...
                        R = ab.apply(Ra, B);
                        o.align(r, false, {
                            x: sa, y: R, width: Ca.apply(Ra, F) - sa, height: Ca.apply(Ra,
                                B) - R
                        });
                        o.show()
                    } else o && o.hide();
            ...
            ra = function (l, p, r) {
                var o, V, W;
                l = ma(l, null, null, r);
                var F = r && Mc || La, A = r && hd || Ta, ea;
                r = V = T(l + ua);
                o = W = T(F - l - ua);
                if (isNaN(l)) ea = true; else if (L) {
                    o = Z;
                    W = F - ob;
            ...
            oa(s, {
                addPlotBand: ta, addPlotLine: ta, adjustTickAmount: function () {
                    if (Fb && !J && !gb && !Nb) {
                        var l = ec, p = la.length;
                        ec = Fb[G];
                        if (p < ec) {
                ...
                }, categories: gb, getExtremes: function () {
                    return {min: H, max: P, dataMin: Qa, dataMax: v}
                }, getPlotLinePath: ra, getThreshold: function (l) {
                    if (H > l) l = H; else if (P < l) l = P;
                    return ma(l, 0, 1)
                }, isXAxis: ha,
                options: h, plotLinesAndBands: Ob, getOffset: function () {
                    var l = S.length && M(H) && M(P), p = 0, r = 0, o = h.title, V = h.labels, W = [-1, 1, 1, -1][E];
                    if (!cb) {
                        cb = Y.g("axis").attr({zIndex: 7}).add();
                        Ab = Y.g("grid").attr({zIndex: 1}).add()
                    }
                    fc = 0;
                    if (l || Nb) {
                        t(la, function (A) {
                            if (pb[A]) pb[A].addLabel(); else pb[A] = new x(A);
                            if (E === 0 || E == 2 || {
                                1: "left",
                                3: "right"
                    ...
                    Kb(o, m);
                    r = y(r, true);
                    Ia(s, "setExtremes", {min: l, max: p},
                        function () {
                            wa = l;
                            Sa = p;
                            r && m.redraw()
                        })
                }, setScale: Ea, setTickPositions: ia, translate: ma, redraw: function () {
                ...
                }, removePlotBand: Ha, removePlotLine: Ha, reversed: Xb, stacks: aa
            });
                ...
                add: function (h, x, w, O) {
                    if (!m[h]) {
                        x = Y.text(x, 0, 0).css(a.toolbar.itemStyle).align({
                            ...
                            x: -zb - 20,
                            y: Z + 30
                        }).on("click", O).attr({align: "right", zIndex: 20}).add();
                        m[h] = x
                    }
                },
                remove: function (h) {
        ...
        function e(m) {
            function h() {
                var G = this.points || mc(this.point), C = G[0].series.xAxis, D = this.x;
            ...
            var O, fa = m.borderWidth, I = m.crosshairs, ia = [], Ea = m.style, ta = m.shared, Ya = pa(Ea.padding),
                Ha = fa + Ya, ha = true, Ka, L, E = 0, aa = 0;
            Ea.padding = 0;
            var s = Y.g("tooltip").attr({zIndex: 8}).add(), J = Y.rect(Ha, Ha, 0, 0, m.borderRadius, fa).attr({
                    fill: m.backgroundColor,
                    "stroke-width": fa
                }).add(s).shadow(m.shadow),
                Q = Y.text("", Ya + Ha, pa(Ea.fontSize) + Ya + Ha).attr({zIndex: 1}).css(Ea).add(s);
            s.hide();
            return {
                shared: ta, refresh: function (G) {
                    var C, D, ba, ua = 0, ma = {}, ra = [];
                    ba = G.tooltipPos;
                    C = m.formatter || h;
                    ma = q.hoverPoints;
                    var cb = function (Fa) {
                    ...
                    if (ma === false || !ba) w(); else {
                        ...
                        if (D < 5) D = 5; else if (D + L > La) D = La - L - 5;
                        x(T(C - Ha), T(D - Ha))
                    }
                    if (I) {
                        I = mc(I);
                        ...
                        for (var Ab; D--;) if (I[D] &&
                            ...
                            if (ia[D]) ia[D].attr({d: C, visibility: Bb}); else {
                                ba = {"stroke-width": I[D].width || 1, stroke: I[D].color || "#C0C0C0", zIndex: 2};
        ...
        function f(m, h) {
            function x(E) {
                ...
                if (E.type != "mousemove" || rb.opera) {
                    for (var s = xa, J = {left: s.offsetLeft, top: s.offsetTop}; s = s.offsetParent;) {
                        J.left +=
                            s.offsetLeft;
                        J.top += s.offsetTop;
            ...
            function O() {
                var E = m.hoverSeries, aa = m.hoverPoint;
                aa && aa.onMouseOut();
                E && E.onMouseOut();
                qc && qc.hide();
                jd = null
            ...
            function fa() {
                if (ta) {
                    var E = {xAxis: [], yAxis: []}, aa = ta.getBBox(), s = aa.x - U, J = aa.y - Z;
                    if (Ea) {
                        t(Xa, function (Q) {
                            var G = Q.translate, C = Q.isXAxis, D = Ga ? !C : C,
                                ba = G(D ? s : qa - J - aa.height, true);
                            G = G(D ? s + aa.width : qa - J, true);
                            E[C ? "xAxis" : "yAxis"].push({axis: Q, min: ab(ba, G), max: Ca(ba, G)})
                        });
                        Ia(m, "selection", E, kd)
                    }
                    ta = ta.destroy()
            ...
            var I, ia, Ea, ta, Ya = u.zoomType, Ha = /x/.test(Ya), ha = /y/.test(Ya), Ka = Ha && !Ga || ha &&
                Ga, L = ha && !Ga || Ha && Ga;
            Nc = function () {
                if (Oc) {
                    Oc.translate(U, Z);
                    Ga && Oc.attr({width: m.plotWidth, height: m.plotHeight}).invert()
                } else m.trackerGroup = Oc = Y.g("tracker").attr({zIndex: 9}).add()
            };
            Nc();
            ...
            (function () {
                ...
                var aa = function (s) {
                    if (!(s && s.touches && s.touches.length > 1)) {
                        ...
                        var J = s.chartX, Q = s.chartY, G = !hc(J - U, Q - Z);
                        if (Gb && s.type ==
                            "touchstart") if (ya(s.target, "isTracker")) m.runTrackerClick || s.preventDefault(); else !$d && !G && s.preventDefault();
                        if (G) {
                            E || O();
                            if (J < U) J = U; else if (J > U + Ba) J = U + Ba;
                            if (Q < Z) Q = Z; else if (Q > Z + qa) Q = Z + qa
                        }
                        if (ld && s.type != "touchstart") {
                            if (Ea = Math.sqrt(Math.pow(I - J, 2) + Math.pow(ia - Q, 2)) > 10) {
                                ...
                                if (ta && Ka) {
                                    J = J - I;
                                    ta.attr({width: $a(J), x: (J > 0 ? 0 : J) + I})
                                }
                                if (ta && L) {
                                    Q = Q - ia;
                                    ta.attr({height: $a(Q), y: (Q > 0 ? 0 : Q) + ia})
                                }
                            }
                        } else if (!G) {
                            var C;
                            Q = m.hoverPoint;
                            J = m.hoverSeries;
                            var D, ba, ua = Ta, ma = Ga ? s.chartY : s.chartX - U;
                            if (qc && h.shared) {
                                C = [];
                                D = za.length;
                                for (ba = 0; ba < D; ba++) if (za[ba].visible && za[ba].tooltipPoints.length) {
                ...
                xa.onclick = function (s) {
                    ...
                    s = x(s);
                    s.cancelBubble = true;
                    if (!Ea) if (J && ya(s.target, "isTracker")) {
                        var Q = J.plotX, G = J.plotY;
                        oa(J, {pageX: pc.left + U + (Ga ? Ba - G : Q), pageY: pc.top + Z + (Ga ? qa - Q : G)});
                        Ia(J.series, "click", oa(s, {point: J}));
            ...
            oa(this, {zoomX: Ha, zoomY: ha, resetTracker: O})
        }

        function g(m) {
            var h =
                m.type || u.type || u.defaultSeriesType, x = sb[h], w = q.hasRendered;
            if (w) if (Ga && h == "column") x = sb.bar; else if (!Ga && h == "bar") x = sb.column;
            h = new x;
            h.init(q, m);
        ...
        function j(m) {
            var h = q.isDirtyLegend, x, w = q.isDirtyBox, O = za.length, fa = O, I = q.clipRect;
            for (Kb(m, q); fa--;) {
                m = za[fa];
            ...
            if (w) {
                ...
                if (I) {
                    Qc(I);
                    I.animate({width: q.plotSizeX, height: q.plotSizeY})
                }
            }
            t(za, function (ia) {
        ...
        function n(m, h) {
            kc = va(a.title, m);
            sc = va(a.subtitle, h);
            t([["title", m, kc], ["subtitle", h, sc]], function (x) {
                var w = x[0], O = q[w], fa = x[1];
                x = x[2];
                if (O && fa) {
                    O.destroy();
                    O = null
        ...
        function z() {
            ...
            if (!ib.offsetWidth) {
                Rb = ib.cloneNode(0);
                Ua(Rb, {position: lc, top: "-9999px", display: ""});
            ...
            q.container = xa = fb(Lb, {
                    ...
                    id: Od
                }, oa({position: Pd, overflow: tb, width: Ta + bb, height: La + bb, textAlign: "left"}, u.style),
                Rb || ib);
            ...
            if (/Gecko/.test(vc)) {
                m = function () {
                    ...
                    Ua(xa, {left: -h.left % 1 + bb, top: -h.top % 1 + bb})
                };
                m();
                Ma(rb, "resize", m);
        ...
        function K() {
            function m() {
                var x = u.width || ib.offsetWidth, w = u.height || ib.offsetHeight;
                if (x && w) {
                    if (x != Rc || w != uc) {
                        clearTimeout(h);
        ...
        function ja() {
            ...
            if (!q.seriesGroup) q.seriesGroup = Y.g("series-group").attr({zIndex: 3}).add();
            t(za, function (w) {
                w.translate();
                w.setTooltipPoints();
                w.render()
            ...
            m.items && t(m.items, function () {
                var w = oa(m.style, this.style), O = pa(w.left) + U, fa = pa(w.top) + Z + 12;
                delete w.left;
                delete w.top;
                Y.text(this.html, O, fa).attr({zIndex: 2}).css(w).add()
            });
        ...
        function $() {
            var m = za.length, h = xa.parentNode;
            Ia(q, "destroy");
            Cb(rb, "unload", $);
            Cb(q);
            for (t(Xa, function (x) {
        ...
        a = va(Pa, a);
        var u = a.chart, N = u.margin;
        N = Jb(N) ? N : [N, N, N, N];
        var ka = y(u.marginTop, N[0]),
            Na = y(u.marginRight, N[1]), ca = y(u.marginBottom, N[2]), na = y(u.marginLeft, N[3]), Za = u.spacingTop,
            jb = u.spacingRight, sd = u.spacingBottom, Uc = u.spacingLeft, tc, kc, sc, Z, zb, ob, U, Qb, ib, Rb, xa, Od,
            Rc, uc, Ta, La, hd, Mc, td, ud, vd, wd, q = this, $d = (N = u.events) && !!N.click, xd, hc, qc, ld, $b, Rd,
            yd, qa, Ba, gc, Oc, Nc, md, Sb, Tb, pc, ic = u.showAxes, Pc = 0, Xa = [], Fb, za = [], Ga, Y, id, Nd, jd,
            nd, rc, qd, rd, pd, kd, Sd, ae = function (m) {
                function h(v, S) {
                    var wa = v.legendItem, Sa = v.legendLine, P = v.legendSymbol, H = L.color,
                        hb = S ? I.itemStyle.color : H;
                    H = S ? v.color : H;
                    wa && wa.css({fill: hb});
                    Sa && Sa.attr({stroke: H});
                    P && P.attr({stroke: H, fill: H})
                }

                function x(v, S, wa) {
                    var Sa = v.legendItem, P = v.legendLine, H = v.legendSymbol;
                    v = v.checkbox;
                    Sa && Sa.attr({x: S, y: wa});
                    P && P.translate(S, wa - 4);
                    H && H.attr({x: S + H.xOff, y: wa + H.yOff});
                    if (v) {
                        v.x = S;
                        v.y = wa
                    }
                ...
                function O(v) {
                    ...
                    if (!hb) {
                        ...
                        v.legendItem =
                            ...
                            }).attr({zIndex: 2}).add(ra);
                        if (!H && v.options && v.options.lineWidth) {
                            var cc = v.options;
                            P = {"stroke-width": cc.lineWidth, zIndex: 2};
                        ...
                        if (H) S = Y.rect(wa = -Ea - ta, Sa = -11, Ea, 12, 2).attr({
                            "stroke-width": 0,
                            zIndex: 3
                        }).add(ra); else if (v.options && v.options.marker && v.options.marker.enabled) S = Y.symbol(v.symbol, wa = -Ea / 2 - ta, Sa = -4, v.options.marker.radius).attr(v.pointAttr[db]).attr({zIndex: 3}).add(ra);
                        if (S) {
                            S.xOff = wa;
                            S.yOff = Sa
                        }
                        ...
                        if (v.options && v.options.showCheckbox) {
                            ...
                            Ma(v.checkbox, "click", function (Vb) {
                                Ia(v, "checkboxClick", {checked: Vb.target.checked}, function () {
                                    v.select()
                                })
                            })
                        }
                ...
                function fa() {
                    ...
                    C = cb = 0;
                    Ya = [];
                    ra || (ra = Y.g("legend").attr({zIndex: 7}).add());
                    Qa && Fa.reverse();
                    t(Fa, function (Sa) {
                    ...
                    ra.align(oa(I, {width: Sb, height: Tb}), true, tc);
                    Pc || w()
                }

                var I = m.options.legend;
                if (I.enabled) {
                    var ia = I.layout == "horizontal", Ea = I.symbolWidth, ta = I.symbolPadding, Ya, Ha = I.style,
                        ha = I.itemStyle, Ka = I.itemHoverStyle, L = I.itemHiddenStyle, E = pa(Ha.padding), aa = 20, s = 18,
                        J = 4 + E + Ea + ta, Q, G, C, D = 0, ba, ua = I.borderWidth, ma = I.backgroundColor, ra, cb,
                        Ab = I.width, Fa = m.series, Qa = I.reversed;
                    fa();
                    Ma(m, "endResize", w);
                    return {
                        colorizeItem: h, destroyItem: function (v) {
                            var S = v.checkbox;
        ...
        Sd = function () {
            Ia(q, "selection", {resetSelection: true}, kd);
            q.toolbar.remove("zoom")
        };
        kd = function (m) {
            var h = Pa.lang, x = q.pointCount < 100;
        ...
        rc = function () {
            var m = a.legend, h = y(m.margin, 10), x = m.x, w = m.y, O = m.align, fa = m.verticalAlign,
                I;
            qd();
            if ((q.title || q.subtitle) && !M(ka)) if (I = Ca(q.title && !kc.floating && !kc.verticalAlign && kc.y || 0, q.subtitle && !sc.floating && !sc.verticalAlign && sc.y || 0)) Z = Ca(Z, I + y(kc.margin, 15) + Za);
            if (m.enabled && !m.floating) if (O == "right") M(Na) || (zb = Ca(zb, Sb - x + h + jb)); else if (O == "left") M(na) || (U = Ca(U, Sb + x + h + Uc)); else if (fa == "top") M(ka) || (Z = Ca(Z, Tb + w + h + Za)); else if (fa == "bottom") M(ca) || (ob = Ca(ob, Tb - w + h + sd));
            ic && t(Xa, function (ia) {
                ia.getOffset()
            });
            M(na) || (U += Qb[3]);
        ...
        pd = function (m, h, x) {
            ...
            Ta = T(m);
            La = T(h);
            Ua(xa, {width: Ta + bb, height: La + bb});
            Y.setSize(Ta, La);
            Ba = Ta - U - zb;
            qa = La - Z - ob;
            Fb = null;
        ...
        rd = function () {
            ...
            tc = {x: Uc, y: Za, width: Ta - Uc - jb, height: La - Za - sd}
        };
        qd = function () {
            Z = y(ka, Za);
            zb = y(Na, jb);
        ...
        nd = function () {
            var m = u.borderWidth || 0, h = u.backgroundColor, x = u.plotBackgroundColor, w = u.plotBackgroundImage, O,
                fa = {x: U, y: Z, width: Ba, height: qa};
            O = 2 * m + (u.shadow ? 8 : 0);
            if (m || h) if (td) td.animate({
                ...
                "stroke-width": m, fill: h || mb
            }).add().shadow(u.shadow);
            if (x) if (ud) ud.animate(fa); else ud = Y.rect(U, Z, Ba, qa, 0).attr({fill: x}).add().shadow(u.plotShadow);
            if (w) if (vd) vd.animate(fa); else vd = Y.image(w, U, Z, Ba, qa).add();
            if (u.plotBorderWidth) if (wd) wd.animate(fa); else wd = Y.rect(U, Z, Ba, qa, 0, u.plotBorderWidth).attr({
        ...
        q.addSeries = function (m, h, x) {
            ...
            if (m) {
                Kb(x, q);
                h = y(h, true);
                Ia(q, "addSeries", {options: m}, function () {
                    w = g(m);
                    w.isDirty = true;
                    q.isDirtyLegend = true;
                    h && q.redraw()
        ...
        q.get = function (m) {
            var h, x, w;
            for (h = 0; h < Xa.length; h++) if (Xa[h].options.id == m) return Xa[h];
            for (h = 0; h < za.length; h++) if (za[h].options.id == m) return za[h];
        ...
        q.showLoading = function (m) {
            var h = a.loading;
            if (!$b) {
                $b = fb(Lb, {className: "highcharts-loading"}, oa(h.style, {
                    left: U + bb, top: Z + bb, width: Ba + bb, height: qa + bb, zIndex: 10,
                    display: mb
                }), xa);
            ...
            if (!yd) {
                Ua($b, {opacity: 0, display: ""});
                Wc($b, {opacity: h.style.opacity}, {duration: h.showDuration});
                yd = true
            }
        };
        q.pointCount = 0;
    ...
    var Da = document, rb = window, Ra = Math, T = Ra.round, Mb = Ra.floor, dd = Ra.ceil, Ca = Ra.max, ab = Ra.min,
        $a = Ra.abs, ub = Ra.cos, yb = Ra.sin, Ub = Ra.PI, Td = Ub * 2 / 360, vc = navigator.userAgent,
        Zc = /msie/i.test(vc) && !rb.opera, wc = Da.documentMode == 8, be = /AppleWebKit/.test(vc),
        Tc = rb.SVGAngle || Da.implementation.hasFeature("http://www.w3.org/TR/SVG11/feature#BasicStructure",
            "1.1"), Gb = "ontouchstart" in Da.documentElement, Hb, Vc, ce = {}, od = 0, nb = 1, Ec, Pa, Kc, zc, Xc, Oa,
        Lb = "div", lc = "absolute", Pd = "relative", tb = "hidden", Zb = "highcharts-", Bb = "visible", bb = "px",
        mb = "none", Wa = "M", Aa = "L", Ud = "rgba(192,192,192," + (Tc ? 1.0E-6 : 0.0020) + ")", db = "", xb = "hover",
        Ac, $c, ad, bd, nc, Bc, Cc, Cd, Dd, cd, Ed, Fd, eb = rb.HighchartsAdapter, Db = eb || {}, t = Db.each,
        zd = Db.grep, jc = Db.map, va = Db.merge, Ad = Db.hyphenate, Ma = Db.addEvent, Cb = Db.removeEvent,
        Ia = Db.fireEvent, Wc = Db.animate, Qc = Db.stop, sb = {};
    eb && eb.init && eb.init();
    if (!eb && rb.jQuery) {
        var kb =
            jQuery;
        t = function (a, c) {
            for (var b = 0, d = a.length; b < d; b++) if (c.call(a[b], a[b], b, a) === false) return b
        };
        zd = kb.grep;
        jc = function (a, c) {
    ...
    Xc = {
        init: function (a, c, b) {
            c = c || "";
            var d = a.shift, e = c.indexOf("C") > -1, f = e ? 7 : 3, g;
            c = c.split(" ");
            b = [].concat(b);
            ...
            var e = [], f = a.length;
            if (b == 1) e = d; else if (f == c.length && b < 1) for (; f--;) {
                d = parseFloat(a[f]);
    ...
    eb = {
        enabled: true,
        align: "center", x: 0, y: 15, style: {color: "#666", fontSize: "11px"}
    };
    Pa = {
        ...
        labels: {style: {position: lc, color: "#3E576F"}},
        legend: {
            enabled: true,
            align: "center",
            layout: "horizontal",
            ...
            itemCheckboxStyle: {position: lc, width: "13px", height: "13px"},
            symbolWidth: 16,
            symbolPadding: 5,
        ...
        loading: {
            hideDuration: 100, labelStyle: {fontWeight: "bold", position: Pd, top: "1em"}, showDuration: 100,
            style: {position: lc, backgroundColor: "white", opacity: 0.5, textAlign: "center"}
        },
        tooltip: {
            enabled: true,
    ...
    var Jc = {
        ...
        }), Yd = {labels: {align: "right", x: -8, y: 3}, title: {rotation: 270}},
        Xd = {labels: {align: "left", x: 8, y: 3}, title: {rotation: 90}},
        Ld = {labels: {align: "center", x: 0, y: 14}, title: {rotation: 0}},
        Wd = va(Ld, {labels: {y: -5}}), vb = Pa.plotOptions;
    eb = vb.line;
    vb.spline = va(eb);
    vb.scatter = va(eb, {lineWidth: 0, states: {hover: {lineWidth: 0}}});
    vb.area = va(eb, {});
    ...
    vb.bar = va(vb.column, {dataLabels: {align: "left", x: 5, y: 0}});
    vb.pie = va(eb, {
        borderColor: "#FFFFFF",
        borderWidth: 1,
        center: ["50%", "50%"],
    ...
    var ac = function (a) {
        var c = [], b;
        (function (d) {
            if (b = /rgba\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]?(?:\.[0-9]+)?)\s*\)/.exec(d)) c = [pa(b[1]), pa(b[2]), pa(b[3]), parseFloat(b[4], 10)]; else if (b = /#([a-fA-F0-9]{2})([a-fA-F0-9]{2})([a-fA-F0-9]{2})/.exec(d)) c =
                [pa(b[1], 16), pa(b[2], 16), pa(b[3], 16), 1]
        })(a);
        return {
            get: function (d) {
    ...
    Kc = function (a, c, b) {
        ...
        c = new Date(c *
            nb);
        var e = c[ad](), f = c[bd](), g = c[nc](), i = c[Bc](), j = c[Cc](), k = Pa.lang, n = k.weekdays;
        k = k.months;
        c = {
            a: n[f].substr(0, 3),
            A: n[f],
    ...
    Fc.prototype = {
        ...
        animate: function (a, c, b) {
            ...
            var b, d, e, f, g = this.element, i = g.nodeName, j = this.renderer, k, n = this.shadows, z, K = this;
            if (Ib(a) && M(c)) {
                b = a;
                a = {};
                a[b] = c
            ...
            if (Ib(a)) {
                b = a;
                if (i == "circle") b = {x: "cx", y: "cy"}[b] || b; else if (b == "strokeWidth") b = "stroke-width";
                K = ya(g, b) || this[b] || 0;
                ...
                } else if (b == "fill") d = j.color(d, g, b); else if (i == "circle" && (b == "x" || b == "y")) b = {
                    x: "cx",
                    y: "cy"
                }[b] || b; else if (b == "translateX" || b == "translateY" || b == "rotation") {
                    this[b] = d;
                    this.updateTransform();
                    k = true
                } else if (b == "stroke") d = j.color(d, g, b); else if (b == "dashstyle") {
                    b = "stroke-dasharray";
                    if (d) {
                        d =
                ...
                } else if (b == "isTracker") this[b] = d; else if (b == "width") d = pa(d); else if (b == "align") {
                    b = "text-anchor";
                ...
                if (n && /^(width|height|visibility|x|y|d)$/.test(b)) for (e = n.length; e--;) ya(n[e], b, d);
                if (b == "text") j.buildText(g, d); else k || ya(g, b, d)
            }
            return K
            ...
            var a = this.translateX || 0, c = this.translateY || 0, b = this.inverted, d = this.rotation, e = [];
            if (b) {
                a += this.attr("width");
                c += this.attr("height")
            }
            if (a || c) e.push("translate(" + a + "," + c + ")");
            if (b) e.push("rotate(90) scale(-1,1)"); else d && e.push("rotate(" + d + " " + this.x + " " + this.y + ")");
        ...
        toFront: function () {
            ...
            b = y(b, this.renderer);
            var d = a.align, e = a.verticalAlign, f = (b.x || 0) + (a.x || 0), g = (b.y || 0) + (a.y || 0), i = {};
            if (/^(right|center)$/.test(d)) f += (b.width - (a.width || 0)) / {right: 1, center: 2}[d];
            i[c ? "translateX" : "x"] = f;
            ...
            var a, c, b, d = this.rotation, e = d * Td;
            try {
            ...
            return this.attr({visibility: Bb})
        }, hide: function () {
            return this.attr({visibility: tb})
        }, add: function (a) {
            var c = this.renderer, b = a || c;
            c = b.element || c.box;
            var d = c.childNodes, e = this.element, f = ya(e, "zIndex"), g;
            this.parentInverted = a && a.inverted;
            if (f) {
                b.handleZ = true;
                f = pa(f)
            }
            if (b.handleZ) for (g = 0; g < d.length; g++) {
                a = d[g];
                b = ya(a, "zIndex");
            ...
            var c = [], b, d = this.element, e = this.parentInverted ? "(-1,-1)" : "(1,1)";
            if (a) {
                for (a = 1; a <= 3; a++) {
                    b = d.cloneNode(0);
                    ya(b, {
    ...
    Sc.prototype = {
        init: function (a, c, b) {
            ...
            t(b, function (k, n) {
                ...
                t(z, function ($) {
                    if ($ !== "" || z.length == 1) {
                        var ga = {}, u = Da.createElementNS("http://www.w3.org/2000/svg", "tspan");
            ...
            return this.symbol("arc", a || 0, c || 0, b || 0, {innerR: d || 0, start: e || 0, end: f || 0})
            ...
            g = Jb(a) ? a : {x: a, y: c, width: Ca(b, 0), height: Ca(d, 0)};
            return this.createElement("rect").attr(oa(g, {rx: e || g.r, ry: e || g.r, fill: mb}))
        },
        setSize: function (a, c, b) {
            var d = this.alignedObjects, e = d.length;
            this.width = a;
            this.height = c;
            for (this.boxWrapper[y(b, true) ? "animate" : "attr"]({width: a, height: c}); e--;) d[e].align()
        }, g: function (a) {
            return this.createElement("g").attr(M(a) && {"class": Zb + a})
        }, image: function (a, c, b, d, e) {
            var f = {preserveAspectRatio: mb};
            arguments.length > 1 && oa(f, {x: c, y: b, width: d, height: e});
            ...
            if (g) {
                f = this.path(g);
                oa(f, {symbolName: a, x: c, y: b, r: d});
                e && oa(f, e)
            } else if (i.test(a)) {
                a = a.match(i)[1];
                f = this.image(a).attr({x: c, y: b});
                fb("img", {
                    onload: function () {
                        var j = ce[this.src] || [this.width, this.height];
                        f.attr({width: j[0], height: j[1]}).translate(-T(j[0] / 2), -T(j[1] / 2))
                    }, src: a
                })
            ...
            var e = Zb + od++, f = this.createElement("clipPath").attr({id: e}).add(this.defs);
            ...
            c = T(y(c, 0));
            b = T(y(b, 0));
            a = this.createElement("text").attr({x: c, y: b, text: a}).css({
    ...
    if (!Tc) {
        var fe = wb(Fc, {
            init: function (a, c) {
                var b = ["<", c, ' filled="f" stroked="f"'], d = ["position: ", lc, ";"];
                ...
                var c = this.renderer, b = this.element, d = c.box;
                d = a ? a.element || a : d;
                a && a.inverted && c.invertChild(b, d);
                wc && d.gVis == tb && Ua(b, {visibility: tb});
                d.appendChild(b);
                this.added = true;
                this.alignOnAdd && this.updateTransform();
                return this
            }, attr: function (a, c) {
                var b, d, e, f = this.element || {}, g = f.style, i = f.nodeName, j = this.renderer,
                    k = this.symbolName, n, z, K = this.shadows, ja = this;
                if (Ib(a) && M(c)) {
                    b = a;
                    a = {};
                    a[b] = c
                ...
                if (Ib(a)) {
                    ...
                    if (k && /^(x|y|r|start|end|width|height|innerR)/.test(b)) {
                        ...
                        d = n.join(" ") || "x";
                        f.path = d;
                        if (K) for (e = K.length; e--;) K[e].path = d;
                        n = true
                    } else if (b == "zIndex" || b == "visibility") {
                        if (wc && b == "visibility" && i == "DIV") {
                            f.gVis = d;
                            n = f.childNodes;
                            for (e = n.length; e--;) Ua(n[e], {visibility: d});
                            if (d == Bb) d = null
                        }
                        if (d) g[b] = d;
                        n = true
                        ...
                        this[b] = d;
                        if (f.tagName == "SPAN") this.updateTransform(); else g[{x: "left", y: "top"}[b]] = d
                    } else if (b == "class") f.className = d; else if (b == "stroke") {
                        d = j.color(d, f, b);
                        b = "strokecolor"
                        ...
                        this.dashstyle = d;
                        n = true
                    } else if (b == "fill") if (i == "SPAN") g.color = d; else {
                    ...
                    if (K && b == "visibility") for (e = K.length; e--;) K[e].style[b] = d;
                    if (!n) if (wc) f[b] = d; else ya(f, b, d)
                }
                return ja
            },
            clip: function (a) {
                var c = a.members, b = c.length;
                c.push(this);
                ...
                if (this.added) {
                    var a = this, c = a.element, b = a.translateX || 0, d = a.translateY || 0, e = a.x || 0,
                        f = a.y || 0, g = a.rotation || 0, i = g * Td, j = ub(i);
                    i = yb(i);
                    var k = a.textAlign || "left", n = {right: 1, center: 2}[k], z = k && k != "left";
                    if (b || d) a.css({marginLeft: b, marginTop: d});
                    ...
                    if (c.tagName == "SPAN") {
                        ...
                        Ua(c, {textAlign: k, left: e, top: f})
                    }
                } else this.alignOnAdd = true
            }, shadow: function (a) {
                var c = [], b = this.element, d = this.renderer, e, f = b.style, g, i = b.path;
                if ("" + b.path === "") i = "x";
                if (a) {
                    for (a = 1; a <= 3; a++) {
                        g = ['<shape isShadow="true" strokeweight="',
                            7 - 2 * a, '" filled="false" path="', i, '" coordsize="100,100" style="', b.style.cssText, '" />'];
                        e = fb(d.prepVML(g), null, {left: pa(f.left) + 1, top: pa(f.top) + 1});
        ...
        Ja.prototype = va(Sc.prototype, {
            isIE8: vc.indexOf("MSIE 8.0") > -1, init: function (a, c, b) {
                var d;
                this.Element = fe;
                this.alignedObjects = [];
                ...
                var d = Pa.chart.style;
                return this.createElement("span").attr({text: a, x: T(c), y: T(b)}).css({
                    whiteSpace: "nowrap",
                ...
                var c;
                if (a) c = {className: Zb + a, "class": Zb + a};
                return this.createElement(Lb).attr(c)
            },
            image: function (a, c, b, d, e) {
                var f = this.createElement("img").attr({src: a});
                arguments.length > 1 && f.css({left: c, top: b, width: d, height: e});
                return f
            }, rect: function (a, c, b, d, e, f) {
                if (arguments.length > 1) {
                    var g = (f || 0) % 2 / 2;
                ...
                return this.symbol("rect", a || 0, c || 0, e || 0, {width: b || 0, height: d || 0})
            }, invertChild: function (a, c) {
                var b = c.style;
                Ua(a, {flip: "x", left: pa(b.width) - 10, top: pa(b.height) - 10, rotation: -90})
            }, symbols: {
                arc: function (a,
                               c, b, d) {
                    var e = d.start, f = d.end, g = ub(e), i = yb(e), j = ub(f), k = yb(f);
                    d = d.innerR;
                    if (f - e === 0) return ["x"]; else if (f - e == 2 * Ub) j = -0.07 / b;
    ...
    xc.prototype = {
        init: function (a, c) {
            ...
            var c = this.series;
            this.config = a;
            if (bc(a) || a === null) this.y = a; else if (Jb(a) &&
                !bc(a.length)) {
                oa(this, a);
                this.options = a
            } else if (Ib(a[0])) {
            ...
            var b = this, d = b.series, e = d.chart, f = d.data;
            Kb(c, e);
            a = y(a, true);
            ...
            if (!this.hasImportedEvents) {
                var a = va(this.series.options.point, this.options).events, c;
                this.events = a;
                for (c in a) Ma(this, c, a[c]);
                this.hasImportedEvents = true
            }
        }, setState: function (a) {
            var c = this.series, b = c.options.states, d = vb[c.type].marker && c.options.marker, e = d && !d.enabled,
                f = (d = d && d.states[a]) && d.enabled === false, g = c.stateMarkerGraphic, i = c.chart,
                j = this.pointAttr;
            a || (a =
                db);
    ...
    lb.prototype = {
        ...
        autoIncrement: function () {
            var a = this.options, c = this.xIncrement;
        ...
        cleanData: function () {
            var a = this.chart, c = this.data, b, d, e = a.smallestInterval, f, g;
            c.sort(function (i, j) {
                return i.x - j.x
            });
            for (g = c.length - 1; g >= 0; g--) c[g - 1] && c[g - 1].x == c[g].x && c.splice(g - 1, 1);
            for (g = c.length - 1; g >= 0; g--) if (c[g - 1]) {
                f = c[g].x - c[g - 1].x;
                if (d === Oa || f < d) {
                    d = f;
                    b = g
        ...
        setData: function (a, c) {
            var b = this, d = b.data, e = b.initialColor, f = b.chart, g = d && d.length || 0;
            b.xIncrement = null;
            if (M(e)) Hb = e;
        ...
        setTooltipPoints: function (a) {
            var c = this.chart, b = c.inverted, d = [], e = T((b ? c.plotTop : c.plotLeft) + c.plotSizeX), f, g, i = [];
        ...
        animate: function (a) {
            var c = this, b = c.chart, d = c.clipRect, e = c.options.animation;
            if (e && !Jb(e)) e = {};
            if (a) {
                if (!d.isAnimating) {
                    d.attr("width", 0);
                ...
                d.animate({width: b.plotSizeX}, e && oa(e, {
                    complete: function () {
                        d.isAnimating = false;
                        if (d != b.clipRect) {
        ...
        drawPoints: function () {
            var a, c = this.data, b = this.chart, d, e, f, g, i, j;
            if (this.options.marker.enabled) for (f = c.length; f--;) {
                g = c[f];
                d = g.plotX;
                e = g.plotY;
                j = g.graphic;
        ...
        convertAttribs: function (a, c, b, d) {
            var e = this.pointAttrToOptions, f, g, i = {};
            a = a || {};
            c = c || {};
            b = b || {};
            d = d || {};
        ...
        getAttribs: function () {
            var a = this, c = vb[a.type].marker ? a.options.marker : a.options, b = c.states, d = b[xb], e, f = {},
                g = a.color,
            ...
            for (f = i.length; f--;) {
                ...
                e = false;
                if (g.options) for (var z in n) if (M(c[n[z]])) e = true;
                if (e) {
                    k =
                        [];
                    b = c.states || {};
        ...
        destroy: function () {
            var a = this, c = a.chart, b = /\/5[0-9\.]+ Safari\//.test(vc), d, e;
            Cb(a);
        ...
        drawDataLabels: function () {
            if (this.options.dataLabels.enabled) {
                var a = this, c, b, d = a.data, e = a.options.dataLabels, f, g = a.dataLabelsGroup, i = a.chart,
                    j = i.inverted, k = a.type, n;
                ...
                t(d, function (z) {
                    var K = y(z.barX, z.plotX, -999), ja = y(z.plotY, -999), $ = z.dataLabel, ga = e.align;
                    f = e.formatter.call({
                        x: z.x,
                        y: z.y,
                        series: a,
                    ...
                    if (k == "column") c += {center: z.barW / 2, right: z.barW}[ga] || 0;
                    if ($) $.animate({x: c, y: b}); else if (f) z.dataLabel = i.renderer.text(f, c, b).attr({
                        align: ga,
                        rotation: e.rotation,
                        zIndex: 1
                    }).css(e.style).add(g)
        ...
        drawGraph: function () {
            var a = this, c = a.options, b = a.graph, d = [],
                e, f = a.area, g = a.group, i = c.lineColor || a.color, j = c.lineWidth, k = c.dashStyle, n,
                z = a.chart.renderer, K = a.yAxis.getThreshold(c.threshold || 0), ja = /^area/.test(a.type), $ = [],
                ga = [];
            t(a.segments, function (u) {
                n = [];
                t(u, function (ca, na) {
                    if (a.getPointSpline) n.push.apply(n, a.getPointSpline(u, ca, na)); else {
                        n.push(na ? Aa : Wa);
                        na && c.step && n.push(ca.plotX, u[na - 1].plotY);
                        n.push(ca.plotX, ca.plotY)
                    }
                });
                if (u.length > 1) d = d.concat(n); else $.push(u[0]);
                if (ja) {
                    var N = [], ka, Na = n.length;
                    for (ka = 0; ka < Na; ka++) N.push(n[ka]);
                    Na == 3 && N.push(Aa,
                        n[1], n[2]);
                    if (c.stacking && a.type != "areaspline") for (ka = u.length - 1; ka >= 0; ka--) N.push(u[ka].plotX, u[ka].yBottom); else N.push(Aa, u[u.length - 1].plotX, K, Aa, u[0].plotX, K);
                    ga = ga.concat(N)
                }
            });
            a.graphPath = d;
            ...
            if (ja) {
                ...
                if (f) f.animate({d: ga}); else a.area = a.chart.renderer.path(ga).attr({fill: e}).add(g)
            }
            if (b) b.animate({d: d}); else if (j) {
                b = {stroke: i, "stroke-width": j};
                if (k) b.dashstyle = k;
                a.graph = z.path(d).attr(b).add(g).shadow(c.shadow)
            }
        },
        render: function () {
            var a = this.chart, c, b, d = this.options, e = d.animation && this.animate;
            b = a.renderer;
            if (!this.clipRect) {
            ...
            if (!this.group) {
                ...
                if (a.inverted) {
                    b = function () {
                        c.attr({width: a.plotWidth, height: a.plotHeight}).invert()
                    };
                    b();
                    Ma(a, "resize", b)
                }
        ...
        redraw: function () {
            ...
            if (c) {
                ...
                c.animate({translateX: a.plotLeft, translateY: a.plotTop})
            }
            this.translate();
        ...
        setState: function (a) {
            var c = this.options, b = this.graph, d = c.states;
            c = c.lineWidth;
            a = a ||
                db;
            if (this.state != a) {
                ...
                if (!(d[a] && d[a].enabled === false)) {
                    ...
                    if (b && !b.dashstyle) b.attr({"stroke-width": c}, a ? 0 : 500)
                }
            }
        },
        setVisible: function (a, c) {
            var b = this.chart, d = this.legendItem, e = this.group, f = this.tracker, g = this.dataLabelsGroup, i,
                j = this.data, k = b.options.chart.ignoreHiddenSeries;
            i = this.visible;
            i = (this.visible = a = a === Oa ? !i : a) ? "show" : "hide";
            e && e[i]();
            if (f) f[i](); else for (e = j.length; e--;) {
                f = j[e];
        ...
        drawTracker: function () {
            var a = this, c = a.options, b = [].concat(a.graphPath), d = b.length, e = a.chart,
                f = e.options.tooltip.snap, g = a.tracker, i = c.cursor;
            i = i && {cursor: i};
            ...
            if (g) g.attr({d: b}); else a.tracker = e.renderer.path(b).attr({
                isTracker: true,
                stroke: Ud,
                fill: mb,
    ...
    sb.area = Ja;
    Ja = wb(lb, {
        type: "spline", getPointSpline: function (a, c, b) {
            var d = c.plotX, e = c.plotY, f = a[b - 1], g = a[b + 1], i, j, k, n;
            if (b && b < a.length - 1) {
                a = f.plotY;
                k = g.plotX;
                g = g.plotY;
    ...
    var Yc = wb(lb, {
        ...
        translate: function () {
            var a = this, c = a.chart, b = 0, d = a.xAxis.reversed, e = a.xAxis.categories, f = {}, g, i;
            ...
            e = c * j.groupPadding;
            n = (c - 2 * e) / b;
            var z = j.pointWidth, K = M(z) ? (n - z) / 2 : n * j.pointPadding, ja = y(z, n - 2 * K),
                $ = K + (e + ((d ? b - a.columnIndex : a.columnIndex) ||
                    0) * n - c / 2) * (d ? -1 : 1), ga = a.yAxis.getThreshold(j.threshold || 0),
                u = y(j.minPointLength, 5);
            t(k, function (N) {
                var ka = N.plotY, Na = N.yBottom || ga, ca = N.plotX + $, na = dd(ab(ka, Na)), Za = dd(Ca(ka, Na) - na),
                    jb;
                if ($a(Za) < u) {
                    if (u) {
                        Za = u;
                ...
                oa(N, {barX: ca, barY: na, barW: ja, barH: Za});
                N.shapeType = "rect";
                N.shapeArgs = {x: ca, y: na, width: ja, height: Za, r: j.borderRadius};
                N.trackerArgs = M(jb) && va(N.shapeArgs, {height: Ca(6, Za + 3), y: jb})
            })
        },
        getSymbol: function () {
        },
        ...
        drawPoints: function () {
            var a =
                this, c = a.options, b = a.chart.renderer, d, e;
            t(a.data, function (f) {
                var g = f.plotY;
                if (g !== Oa && !isNaN(g)) {
                    d = f.graphic;
        ...
        drawTracker: function () {
            var a = this, c = a.chart, b = c.renderer, d, e, f = +new Date, g = a.options.cursor, i = g && {cursor: g},
                j;
            t(a.data, function (k) {
                e = k.tracker;
                d = k.trackerArgs || k.shapeArgs;
                if (k.y !== null) if (e) e.attr(d); else k.tracker = b[k.shapeType](d).attr({
                    isTracker: f,
                    fill: Ud, visibility: a.visible ? Bb : tb, zIndex: 1
        ...
        animate: function (a) {
            ...
            if (!a) {
                t(b, function (d) {
                    var e = d.graphic;
                    if (e) {
                        e.attr({height: 0, y: c.yAxis.translate(0, 0, 1)});
                        e.animate({
                            height: d.barH,
                            y: d.barY
                        }, c.options.animation)
    ...
    Ja = wb(lb, {
        type: "scatter", translate: function () {
            ...
            var a = this, c = a.options.cursor, b = c && {cursor: c}, d;
            t(a.data, function (e) {
    ...
    Ja = wb(xc, {
        init: function () {
            ...
            var a = this, c;
            oa(a, {visible: a.visible !== false, name: y(a.name, "Slice")});
            c = function () {
                a.slice()
            };
            Ma(a, "select", c);
            ...
            var c = this.series.chart, b = this.tracker, d = this.dataLabel, e = this.connector, f;
        ...
        wb(lb, {
            ...
            animate: function () {
                ...
                t(a.data, function (c) {
                    ...
                    var d = -Ub / 2;
                    if (b) {
                        b.attr({r: 0, start: d, end: d});
                        b.animate({r: c.r, start: c.start, end: c.end}, a.options.animation)
                    }
                });
                a.animate = null
            },
            translate: function () {
                var a = 0, c = -0.25, b = this.options, d = b.slicedOffset, e = d + b.borderWidth, f = b.center,
                    g = this.chart, i = g.plotWidth,
                    j = g.plotHeight, k, n, z, K = this.data, ja = 2 * Ub, $, ga = ab(i, j), u, N, ka,
                    Na = b.dataLabels.distance;
                ...
                t(K, function (ca) {
                    ...
                    n = c * ja;
                    ca.shapeType = "arc";
                    ca.shapeArgs = {x: f[0], y: f[1], r: f[2] / 2, innerR: f[3] / 2, start: k, end: n};
                    z = (n + k) / 2;
            ...
            drawPoints: function () {
                var a = this.chart, c = a.renderer, b, d, e;
                t(this.data, function (f) {
                    d = f.graphic;
                    e = f.shapeArgs;
                    if (!f.group) {
                        b = f.sliced ? f.slicedTranslation : [a.plotLeft, a.plotTop];
                        f.group = c.g("point").attr({zIndex: 5}).add().translate(b[0], b[1])
                    }
                    if (d) d.animate(e); else f.graphic = c.arc(e).attr(f.pointAttr[db]).add(f.group);
                    f.visible === false && f.setVisible(false)
                })
            },
            drawDataLabels: function () {
                var a = this.data, c, b = this.chart, d = this.options.dataLabels, e = y(d.connectorPadding, 10),
                    f = y(d.connectorWidth, 1), g,
                    i, j = d.distance > 0, k, n, z = this.center[1], K = [[], [], [], []], ja, $, ga, u, N, ka, Na,
                    ca = 4, na;
                    ...
                    for (u = 0; u < 2; u++) {
                        ...
                        for (na = 0; na < K[ca].length; na++) {
                            ...
                            if (g = c.dataLabel) {
                                ...
                                k || (k = g &&
                                    g.getBBox().height);
                                if (j) if (u && c.rank < a) ga = tb; else if (!ka && $ < n + k || ka && $ > n - k) {
                                    $ = n + N * k;
                                    ja = this.getX($, ca > 1);
                                    if (!ka && $ + k > z || ka && $ - k < z) if (u) ga = tb; else a++
                                }
                                ...
                                if (u) {
                                    ...
                                    if (j && f) {
                                        ...
                                        if (g) {
                                            ...
                                            "stroke-width": f,
                                            stroke: d.connectorColor || "#606060", visibility: ga, zIndex: 3
